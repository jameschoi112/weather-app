  ...
  const fetchWeatherData = async (lat: number, lon: number) => {
    ...
    try {
      ...
      const weatherData: WeatherData = {
        ...
        hourly: oneCallData.hourly?.slice(0, 6).map((hour: any) => ({
        ...
        daily: oneCallData.daily.slice(0, 7).map((day: any) => ({
...
const AIRecommendation = () => {
   ...
   useEffect(() => {
     const getRecommendation = async () => {
       ...
       try {
         ...
         현재 비나 눈이 오고 있거나, 특정 시간에 강수확률이 20% 이상이라면, 해당 시간대를 언급하면서 우산을 챙겨야 한다고 알려주세요.
         반드시 두 줄로 구성해주시고, 첫 줄은 옷차림, 두 번째 줄은 우산에 대한 내용이어야 합니다.`;
 ...
 const WeeklyForecast = ({ daily }: { daily: DailyForecast[] }) => {
   const today = daily[0];
       ...
       navigator.geolocation.getCurrentPosition(
         async (position) => {
           try {
             ...
             console.error(error);
             setLocation('수원시');
             const weatherData = await fetchWeatherData(37.2911, 127.0089);
             setWeatherData(weatherData);
             setLoading(false);
           }
         },
